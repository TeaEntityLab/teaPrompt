...
| M4 | external-doc-internalization deltas (ephemeral-source workflow, sentinel-fact verification, no-temp-links rule) → `workflow-acquisition.md` | Adopted 2026-07-12 (user-directed; recurrence `unknown`) | `workflow-acquisition.md` §2a Ephemeral-source internalization; guard `plans/tests/test_dormant_conditional_contracts.py` | Revisit if the stanza goes unused across the next internalization sessions |
...
| M6 | README "## Orientation" section (primer extract) | Adopted 2026-07-12 (user-directed; recurrence `unknown`) | Root `README.md` + `README.zh-TW.md` `## Orientation`; guard `plans/tests/test_dormant_conditional_contracts.py` | Trim if it duplicates library README orientation |
| M7 | Redaction methodology (leakage scans, packet hashing, execution metadata) → `external-adoption-review.md` | Adopted 2026-07-12 (user-directed; recurrence `unknown`) | `external-adoption-review.md` §2a Sensitive-evidence packet handling; guard `plans/tests/test_dormant_conditional_contracts.py` | Applies only to real sensitive-evidence reviews; revisit if never exercised |
