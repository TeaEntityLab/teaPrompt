...
| --- | --- | --- | --- | --- | --- |
| SS-1 | The runner's result contract writes `assurance.os_level_confinement: 'not_proven'` and `assurance.remote_provider_cancellation: 'not_proven'`, and the delegation-route validator **requires** `os_confinement === 'not_proven'` — non-proof is a mandatory literal in the shipped contract, not prose. The `--global` hook comment states it runs on every session of that CLI on the machine. | 14:47 provenance + architecture lenses; 20:56 provenance lens | observed (file-cited by two lenses; not re-read here) | "self-declared not an OS sandbox" as prose; `--global` targets without the blast-radius clause | **Recorded.** Convergence: TeaPrompt's `governed-delivery` run note marks host preconditions met / unmet / `unknown` and refuters `unknown` until run; the surveyed project encodes the same unknown as a required field value. This is the sharpest available rebuttal of the talk's "strict sandbox isolation" (author-claimed) and of any reading of TeaPrompt's host-duty word "sandbox" as a capability. |
| SS-2 | Test-count parameter: the *documented* 19-file `node --test` command yields **853 tests / 844 pass / 9 fail, exit 1** (observed in the sibling's command log); this record's 898 / 889 / 9 came from the wider on-disk set (26 `*.test.js`; seven undocumented: completion-context, looper-git, looper-live-gate, notebook-write, queue-routing, streams-off, terminal-preflight). Same nine failures in both. | 14:47 repro + evidence lenses; sibling coordinator log | observed | 898 / 889 / 9 recorded without its file-set parameter | **Correction.** A count is reproducible only with its parameter (GL-8 class: the second method disagreed and exposed an unstated input). Both numbers now stand with their file sets. |
...
This addendum is wrong or must be re-litigated if: (1) a re-run of the documented 19-file command at the pin gives a tally other than 853 / 844 / 9, or the 26-file set gives other than 898 / 889 / 9; (2) the delegation-route validator does not require the `not_proven` literal at the pin; (3) SS-9's recipe sentence is later found to restate an existing line (then it is ceremony and folds back); (4) a future comparison describes dispatch L1 as pit-of-success or guided development; (5) the root README host list and the install guide diverge again without a recorded reason.
