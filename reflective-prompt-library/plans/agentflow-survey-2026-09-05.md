...
> **Status: decided, guarded, and verified — panel record-only, then three clean-room sentences adopted by user direction; entry-point addendum (paste-5) record-only with EP-1/EP-6 deferred.** Seven-lens Parallel Lens Review of `agfnow/agentflow` pinned at `b2935f5381d6469243440e080b43d0092a591663` (2026-09-04) against TeaPrompt's skills and doctrine. Panel outcome: **no in-place wording** (7/7); findings and comparisons recorded; nothing installed or depended upon. A same-day follow-up instruction ("think about update our skills") re-ran the ledger under a "would this make the skill better" objective and adopted AF-2, AF-19, and AF-20 as additive sentences (see *Post-Panel Skill Update Deliberation*). A second same-day external input (a zh-TW entry-point walkthrough) was reviewed by a second seven-lens panel and recorded in the *2026-09-05 Entry-Point Survey Addendum* at the end of this file; it changes no AF row. Authority chain unchanged: `06-repo/AGENTS.md` and the invoked `SKILL.md` contracts govern; this record is evidence and design judgement, not an operating rule. TeaPrompt operates no notebook protocol, Stop hook, pre-commit guard, external runner, or looper.
...
> **Status: decided (record-only for skills), guarded, and verified — scoped addendum; it does not change AF-1–AF-20.** A second external input the same day — a zh-TW entry-point walkthrough of the same pin (`b2935f5381d6469243440e080b43d0092a591663`), origin and generating tool unknown — was reviewed by a seven-lens Parallel Lens Review against packet `review-packet-agentflow-entry-points-2026-09-05.md` (SHA-256 `7f035123004db8ec06ab9595efd46bfd6c325fef5d7b1d50e75030ed743243b0`, 23,130 bytes; TeaPrompt HEAD `2fc377ba13b39a34fd24f8f45ffce9a49ff3db70`, `main`, clean). The sections above are frozen as written earlier that day; every evidence-tier change introduced here is dated in this addendum. Addendum form follows the 2026-08-25 lineage-addendum precedent: same pin, same object, Decision Index rolled into the existing bullet, guard extended rather than re-minted.
...
**Skills: none.** Record corrections (applied here): EP-4 narrowed, EP-5 expanded, EP-9 expanded, packet count and intake argv corrected (below).
...
| ID | Candidate (clean-room) | Status | Evidence | Next action / trigger |
| --- | --- | --- | --- | --- |
| EP-1 | Activation order: read an existing continuation packet or ledger before other discovery; trust a clean result unless it reports a problem or the current request needs more | **Deferred with triggers** 2026-09-05 (panel 2–2 on the gate; record-only under this record's bar) | Absent as text on every installed surface; substance held (`reflective-implement:108,123`, `governed-delivery:32`, `reflective-dispatch:158`, `reflective-handoff-retro:42,81`); no local recurrence | Reopen on (a) a documented TeaPrompt activation that performed broad discovery while a packet or ledger existed unread and produced a wrong route or duplicate work, or (b) an explicit user instruction to re-run the ledger under the same-day "would the installed skill be better?" bar. If adopted: one surface only — `reflective-dispatch` Operating Rules beside `:158` (entry layer, all workflows) or `reflective-implement` Task Packet (the surface a resumed coding turn actually loads); ≤2 additive sentences; the "unless it reports a problem or the request needs more" hatch is load-bearing |
...
| EP-6 | Exercise the real surface a user would use when user-facing behavior changes; reading source or unit tests alone does not satisfy the check | **Deferred with trigger** 2026-09-05 | `reflective-implement:138,139,42`; no shipped journey script on the surveyed side either | Reopen on a documented TeaPrompt delivery that treated source reading or unit tests alone as satisfying the user-facing-behavior bullet; if adopted, verification list only, never a gate precondition (OW-2) |
...
This addendum is wrong or must be re-litigated if: (1) either EP-1 trigger fires and the row is not re-opened; (2) a TeaPrompt skill gains a resume/intake sentence without the "unless it reports a problem or the request needs more" hatch, or gains a no-lighter-route lock, an exact-token trigger, a project config, an init step, or an install pointer to the surveyed repository; (3) EP-6 wording lands on an `execution` or `acceptance` gate; (4) the paste is later quoted as source text rather than re-resolved against the pin; (5) the 5-of-7 reason tally is cited as a 7/7 verdict.
...
| Guard extension | `verified` | `plans/tests/test_agentflow_survey_record.py` addendum tests |
| Decision Index clause, case-study cell | `verified` | `PROJECT_KNOWLEDGE.md`, `external-adoption-case-studies-2026-06-20.md` |
| Repository verification | `verified` | `make all` after the addendum |
