...
Reported results (author-claimed): Lasso path solver — 317 vs 550 calls (Gemini-3.1-Pro) and 1879 vs 3200 (Gemini-3.7-Flash) against the fixed-exploration baseline at better held-out runtime; ~162× fewer calls than SimpleTES's 51,200-generation budget; math optimization — matches or beats baselines within 1k generations (SimpleTES's autocorrelation number stands, at 51,200 generations); KernelBench — 1.79×–2.43× fewer generations at comparable performance, or 1.44×–2.09× better at comparable budget. Analysis: the learned policy is adaptive, not monotonic (cuts attempts 110→50 while improving, re-widens at plateaus); **injected semantic guidance consistently underperformed unguided runs under equal budgets** — strong directional priors over-constrain parallel exploration.
...
| C6 | **Injected semantic guidance underperforms replay** — distilled directional priors over-constrain parallel exploration | prose + one figure (§5.1, ConvDiv, both paradigms) | none — no TeaPrompt surface injects directional guidance into exploration prompts | Deferred (DR-5) |
...
| DR-5 | Caution: distilled directional guidance injected into exploration prompts can underperform unguided runs (their §5.1, one task, both paradigms, equal budgets) | Deferred 2026-09-18 | No installed surface injects such guidance; `artifact-promotion` and `handoff-retro` distill lessons for *quality gates*, not search steering. The finding is one figure on one task — real but narrow | Reopen if a TeaPrompt skill is ever proposed that injects distilled "where to search next" guidance into exploration/dispatch prompts; the caution sentence is reserved: "Directional guidance distilled from prior runs can shrink the space a parallel exploration actually searches; prefer replayable evidence over advice when the budget is parallel." |
...
| arXiv identifier 2609.14858 | Author-claimed | `CITATION.cff`; the repo's own badge still says "coming soon" — not verified against arXiv |
...
- `assets/dream-rsi.pdf` converted text: §1–7 read; appendix solver code skimmed for existence — 2026-09-18.
...
- DR-5's deferral is wrong if a TeaPrompt surface already injects directional guidance into exploration prompts (none found by grep; the trigger row says what reopens it).
