...
- **D4 — deferred.** Record-hygiene lint stays convention-only; all eight audited records passed with only two label fixes, so a validator would currently enforce a standard nothing violates. Revisit trigger: a new record ships without evidence separation or fact-check dating.
