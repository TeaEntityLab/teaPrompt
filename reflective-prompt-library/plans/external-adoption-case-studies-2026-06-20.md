...
| MiniMax Code trace-map survey recorded (pasted zh-TW map, 35 file:line-anchored claims over seven entry-point/engine traces): mechanical verification at the unchanged pin `e3724a13d72d` | seven files fetched raw at the pin; one verifier script; 29 exact / 4 near (uniform +2 README offset; corroborating double-hit) / 1 far (+4) / 1 line-exact paraphrase / 0 fabricated | no wording gap — anchored-claim method already installed (records, guards, intake rule); runtime patterns host territory; zero-IO core and lease broker recorded as corroboration; prior survey's architecture slice upgraded docs→code at the same pin (TM-5) | `plans/minimax-code-trace-map-survey-2026-09-19.md` + `plans/tests/test_minimax_code_trace_map_survey_record.py` |
| Jev-vs-diffusion eval survey recorded (X thread 2026-09-17; `vllm#57250` + two merged prerequisite PRs + `google/diffusiongemma-26B-A4B-it` verified; battery unpublished): rough-tie claim bounded to the battery; both decision surfaces fail multi-step planning even with a scratch register | thread via mirror + GitHub/HF APIs read 2026-09-19; nothing executed | no wording gap — confound disclosure, verdict scoping, upstream-first reporting all installed; planning-boundary finding recorded as corroboration of the fast-path/deliberation split; third decision-interface substrate noted; MV-6 entropy-confidence = DM-5's structural-signal class, unfired | `plans/jev-diffusion-eval-survey-2026-09-19.md` + `plans/tests/test_jev_diffusion_eval_survey_record.py` |
| GE-1 landed under user direction (2026-09-19 rethink over the Jev-decision + MiniMax survey families): the `reflective-research` State Ledger intake sentence installed once after "not a summary of one", byte-identical to the wording reserved 2026-09-16; every other scoped candidate row re-read and unchanged — DM-5/MV-6 structural-signal triggers unfired (tool existence is not host integration); JA-1..9, TM-1..6, MV-1..6 dispositions stand | done | `graph-engineering-synthesis-survey-2026-09-16.md` §Landing Addendum + Decision Index entry + guard flip in `plans/tests/test_graph_engineering_synthesis_survey_record.py` (status Adopted, header regex, sentence pinned once on the skill, Lesson landed-wording) |
| Direction-scope docs pass under generic direction ("Update docs if worth it"): DS-1 direction-scope rule installed once on `04-agent/external-adoption-review.md` §5 (named-family direction fires carried user-direction gates; generic direction fires only the current review's candidates; consideration is not authorization — MC-5/DR-5/GE-1 precedent plus the reverted first GE-1 landing); DS-2 `index.json` staleness-guard sentence on `plans/QUALITY_GATES_SUMMARY.md`; DS-3 no-change (no lens-side State-Ledger mirror; routing surfaces trigger-level) | done | `plans/direction-scope-docs-adoption-2026-09-19.md` + Decision Index entry + guard `plans/tests/test_direction_scope_docs_adoption.py` (DS-1 pinned once inside §5 after the missing-usage rule and absent elsewhere; DS-2 pinned; dispositions; index links) |
| Nimble survey recorded (`bespokelabsai/nimble` @ `d2387fc0b32d`, created one day earlier, no root LICENSE → concepts only; HF adapter `bespokelabs/Bespoke-Nimble-9B` Apache-2.0 at `594dfdcfb6f9`): open Jev-class typed-decision stack — logit-readout scoring over a flat schema, contrastive minimal-pair curation with ablation validity checks, synthetic holdout admitted correlated and answered by a 13-subset human-labeled suite with pre-registered instruments and a rejected-datasets table | GitHub API + recursive tree + five docs read raw at the pin; HF card API; nothing executed — numbers author-claimed, docs-to-code fidelity not audited | no wording gap — ten concepts covered (R11 minimal-pair force, second-method rule at project scale, GW-1 pre-registration), host territory, or corroboration; fifth open decision substrate, first company-backed trained release; DM-5/MV-6 triggers unfired (tool existence is not host integration); same-day generic direction re-ran NB-1..6 — none fire (§Direction Addendum) | `plans/nimble-survey-2026-09-19.md` + `plans/tests/test_nimble_survey_record.py` |
| GLiNER2 survey recorded (`fastino-ai/GLiNER2` @ `d7c727458bf6`, created 2025-07-07, Apache-2.0 at root; EMNLP 2025 demos paper 2025.emnlp-demos.10 read in full; HF `fastino/gliner2.5-base-v1` at `78cea04059`): schema-driven ~200M encoder IE — five task types in one forward pass, runtime-defined labels; constraint-decoded joint typed decisions with explicit infeasibility (first in the surveyed ecosystem); production existence proof of the encoder path the Jev-architecture survey held open (pre-Jev lineage; feasibility ≠ identity) | GitHub API + root tree + README head + constrained-classification tutorial read at the pin; paper PDF in full; HF API; nothing executed — numbers author-claimed, code fidelity not audited | no wording gap — ten concepts covered (packet amortization, judge-lifecycle teacher-label class, verdict-scope disclosure), host territory, or corroboration; G2-2 joint-feasibility sentence no-change with reopen (jointly infeasible coupled verdicts observed locally); paper↔repo drift and encoder existence proof noted record-only; DM-5/MV-6 unfired | `plans/gliner2-survey-2026-09-19.md` + `plans/tests/test_gliner2_survey_record.py` |
| pi-warden survey + goals rethink recorded (`DevMortimer/pi-warden` @ `c9921c3ebc48`, four days old, MIT, v0.29.1): steer-not-interrupt Pi guardrails judged by the Jev decision API over a deterministic floor the model can only raise; calibration-before-enforcement charter (unmeasured questions inert; trace-only until AUC clears; user-reaction labels); approval only from the user's message; prose-vs-warden A/B 6/150 vs 0 author-claimed | GitHub API + seven docs read in full at the pin + floor-spec head; host `judge()` EXECUTED (per-candidate probabilities to a TeaPrompt-run step); nothing else run; code fidelity not audited | PW-1 adopted (DM-5 reopened by the executed trigger; generic direction fires this survey's candidates): confidence-ratchet sentence pinned once on `04-agent/workflow-recipes.md` Confidence row; DM-5 record byte-unedited; goals rethink: north star + runtime non-goals stand, vindicated (warden-class enforcement built host-side, judging TeaPrompt's artifact class); PW-2 parked (rubber-stamp reopen); rest covered/host/corroboration | `plans/pi-warden-survey-2026-09-20.md` + `plans/tests/test_pi_warden_survey_record.py` |
| Fifth-generation prompt-taxonomy rethink recorded (pasted zh-TW claim over the design-by-contract taxonomy — requirements, output contracts, invariants, validation, transformation, knowledge, observation — plus an exclusivity clause): taxonomy verified against sources (arXiv 2409.08775v2 + vendor/third-party 5th-gen guidance); exclusivity clause held by no located source | web verification of taxonomy + Astra/Fable guidance; full library inventory; coaxing sweep over 00-core/01-thinking/02-engineering/05-domain (three benign role headers, zero persona/style content) | record-only — all seven categories already have installed surfaces; exclusivity rejected as category error (procedure layer is harness policy, not prompt content); RT-1..RT-4 no-change with falsifiers; RT-5 records the rethink | `plans/fifth-gen-prompt-taxonomy-rethink-2026-09-21.md` + `plans/tests/test_fifth_gen_taxonomy_rethink_record.py` |
| teaBrain concepts-and-experiments survey recorded (`~/dev/teaBrain` @ `27b4d25`, tree `0788d2c`, clean, 2026-09-21; same-author sibling reflex-control research repo, no-backprop bet, simulation-only, governance artifact-complete): its `docs/governance/` is the `agent-governance-scaffold` artifact set operationalized — four-power split, byte-identical fifteen-invariant vocabulary + `checker_profile` placeholders + L0–L5 ladder, wrapper-contract with `bash` removed, `run-agent.sh` state-predicate interface, exact `artifact-complete` literal — extended with runtime-learner primitives the pack lacks; planning used the parallel-lens panel + falsifiability + walking-skeleton spec-plan | all governance + intents + plans 00/06 + panel + source cores read at the pin; three read-only scouts (plans 01–05, three reviews, code map); experiments EXECUTED in the repo venv — pytest 12/12 and nine benches reproduced their documented numbers (R2 73.0%, R3 65→1→65 vs Braitenberg 78→3, R4 amp_ratio 1.00/circ_var 0.000, R5 62.0% critic vs 11% global); TB-1 pack gap grep-confirmed | record-only (bare survey, DS-1) — twelve concepts covered (governance/thinking/spec corroboration or host territory), no wording adopted; TB-1 runtime-learner governance gap logged not landed (pack scope = coding workers; n=1 same-author usage); TB-3 host-integration trigger not fired (artifact-complete, not enforcement-wired) | `plans/teabrain-concepts-experiments-survey-2026-09-21.md` + Decision Index entry + guard `plans/tests/test_teabrain_survey_record.py` |
| Agentflow 8.3.0 delta survey recorded (`agfnow/agentflow` @ `0abf416`, one squashed commit on the 8.2 pin `fcb6878`; tags/releases empty; "v8.3.0" a file label; config schema 7→8; Apache-2.0): host-implementation-dominated release — portable hookless-host support (verified Codex/Claude vs portable core; `hooks: not_available`; no model/permission by inference), schema-8 `allowed-worker` unordered permission policy, `review-policy: prefer/require-independent`, session-ownership (`agf owner adopt`, sha256, no age/PID expiry), new settings; honesty deltas (no Pi/Gemini/OpenCode live-verification claim; dangling `eval/` load rule removed); one new methodology clause (I-062 challenge mistaken/risky/complex ideas) | GitHub API compare 8.2→8.3 read (CHANGELOG + SKILL.md + README diffs in full); changed-file inventory enumerated; installed-surface vocabulary grep; skill-update re-run grepped the counterargument lens + skills; nothing executed (bare survey → nothing drafted → nothing probed, per the 8.2 reflection) | record-only (bare survey, DS-1) then a skill-update direction re-ran all ten AF83-* at the "would the skill be better" bar — none fired: challenge-user (I-062) is the installed `01-thinking/counterargument` lens (referenced by implement/minimality/review), the rest are host runtime (Standing Non-Goal) or rejected vendor config (AF83-7); prior AF/EP/AF82 adoptions untouched, no contradiction | `plans/agentflow-8.3-delta-survey-2026-09-21.md` + Decision Index entry + guard `plans/tests/test_agentflow_v83_delta_record.py` |
