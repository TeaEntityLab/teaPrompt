...
| 2026-09-05 | agentflow (agfnow/agentflow @ `b2935f5`): devlog notebook protocol, on-demand pipeline, external-runner-v1 no-remote clones, Stop-hook/pre-commit referees, looper live gate, 73-entry incident ledger | Pinned clone; coordinator `node --test` 898/889/9 (9 fail on unshipped `release/` and `eval/` paths); 7-lens review, 7/7 delivered; no script run against a repo, no provider dispatch; private source repo and evals unreachable | no — every methodology invariant already held; remaining mechanisms are operationalization (Standing Non-Goal); inline citations cover 18/73 incidents; recurrence `unknown` | Record-only, `AGREE` 7/7; peer methodology + host harness, not a competitor or host; reject fixed three-start ceiling (ATT-7) and adoption as dependency; no wording | [survey](agentflow-survey-2026-09-05.md) |
...
| agentflow survey recorded (record-only; findings and comparisons) | done | `agentflow-survey-2026-09-05.md` + Case Comparison row + `test_agentflow_survey_record.py` |
