...
| Context-efficiency instruction-profile survey recorded; record-only; two reserved wordings (A-5 read-side twin of I-1, A-7a external-change evidence reuse) with triggers | done | `astra-efficiency-rules-survey-2026-09-10.md` + Decision Index entry + guard `plans/tests/test_astra_efficiency_rules_survey_record.py` (survey tokens and cap literals absent from skills, install guide, GLOSSARY, `04-agent/`; reserved wordings record-only) |
