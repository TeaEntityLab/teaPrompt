...
| 2026-09-13 | Agentflow 8.2.0 public delta (`b2935f5` → `fcb6878`), config schema 7 | API identities + pinned source delta; coordinator audit CLI and 28/28 selected fixture tests; three read-only source slices | Two same-surface wording clarifications, not demonstrated behavioral gaps; four paired synthetic cases had unchanged safe outcomes | AF82-1 conflict resolution on dispatch and AF82-2 protected remedy replacement on minimality adopted by user direction despite no-change source advice; no runtime/install or gate change | [survey](agentflow-8.2-delta-survey-2026-09-13.md) |
...
| Agentflow 8.2.0 delta recorded; two portable clarifications adopted with source-review dissent and no measured efficacy claim | done | `agentflow-8.2-delta-survey-2026-09-13.md` + Decision Index + `test_agentflow_v82_delta_record.py` (identity, dispositions, named-section and single-surface record parity) |
