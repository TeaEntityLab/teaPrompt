...
**F1 — Backlog template (`flow-loop-harness`).** A `snap()` function (the fix loop's signal: tracked diff stat plus untracked count with the worktree-relative `$STATE` excluded; empty outside git) is read before dispatch; a task is retired only when the verifier passes *and* the workspace changed; otherwise `- no change: <task>` and exit 3. Outside git the check is skipped (no signal), matching the fix loop's degrade-gracefully rule. The deviation sentence now states the rule and its cost. Offsets (zero-sum, no rule lost): Verification 1 no longer re-enumerates Anatomy #6's exit codes; Verification 3 no longer restates Anatomy #5; the writer-critic "Caution" no longer restates the Methods stop condition and Never #50; a meta remark ("Guidance, not a new template or runtime"), a dated vendor-product parenthetical and the 2026-07-11 demotion-evaluation pointer in Host-Native Alternatives (the record and the checkpoint runbook carry that result), and the multi-wave "keeps all six parts" claim were dropped; the dispatch comment lost its ledger-tail hint (Anatomy #3 carries that rule). Pack: 19,984 → 19,884.

**Tradeoff, documented after a same-day advisory.** The change gate has two false negatives the loop cannot separate from a chatty no-op: a task already satisfied at dispatch, and a task interrupted after its edits and re-dispatched on resume (its edits sit in `before`). Both previously retired silently and continued; both now halt with `- no change` and exit 3, and the deviation sentence tells the operator what to do (confirm the task, delete its line from `state/TASKS.canon`, rerun — that copy is the resume path). The alternative the advisory named — gate on agent exit 0 plus non-empty output instead of workspace delta — was rejected: it is the agent's self-report, the gate class the pack forbids ("never trust the model's own done"; Methods: model self-reports are advisory only), and it leaves the canonical failure — an agent that prints "done" and edits nothing — retiring every task. The 2026-09-05 note "no progress detector, by design" was checked against its origin (FLH-5, `b8ef926`: "backlog anatomy deviations declared"; the template never had a detector, and no record mentions resume or idempotent tasks): it declared an omission, it did not protect these cases. Halting hands the ambiguity to a human, the pack's stated preference over a soft success.
...
- F1 is wrong if a backlog task legitimately completes without changing the worktree (a pure external effect, an already-satisfied task, or an interrupted task re-dispatched on resume); the loop then exits 3 at that task with an operator instruction and a human decides — the pack's stated preference over a silent retire. F1's balance is wrong if unattended burn-downs halt on these cases more often than the old template falsely retired no-op work; that comparison needs host run logs (`unknown` today).
...
| Both flow packs under budget | done | 19,884 / 19,979 |
...
| Same-day advisory on F1's false negatives (already-satisfied and resumed tasks): tradeoff documented on the template, in Example 2, and above; self-report alternative rejected; FLH-5 origin checked | done | second commit; pack 19,884 |
