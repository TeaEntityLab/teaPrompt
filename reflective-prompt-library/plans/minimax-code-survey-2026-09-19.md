...
- **Adopt: nothing now; one deferral.** The runtime, release pipeline, and CI gates are host territory (P7: TeaPrompt owns no runtime). The epistemic rules its skills encode are already installed — several are point-for-point convergent. The one form sharper than anything installed is the tri-state consumer map (MC-5), deferred with a trigger.
...
| C5 | Tri-state consumer map before completion: enumerate affected consumers, mark `covered` / `not applicable` / `unknown`, never declare complete while `unknown` remains | `reflective-implement` verification contract and the landing-review bullet cover the substance; no installed sentence requires *enumerating* consumer classes with a tri-state before "done" | **Deferred (MC-5)** |
...
| MC-5 | Tri-state consumer map: before declaring an implementation complete, enumerate affected consumer classes and mark each `covered` / `not applicable` / `unknown`; `unknown` blocks completion | Deferred 2026-09-19 | Sharper *form* than installed text: `reflective-implement` requires verifying affected surfaces but never requires enumerating them tri-state; TeaPrompt's own failures of this class were caught by landing reviews, not missed — so no verified local gap today. The form's value is that `unknown` is forced into the record instead of silently absent | Reopen on an observed TeaPrompt-run completion declaration where an affected consumer class was never enumerated (the failure the form prevents); landing surface would be `reflective-implement`'s verification section, additive |
...
- MC-5's deferral is wrong in one direction if a TeaPrompt-run session declares completion with an unenumerated affected consumer — that fires the trigger; wrong in the other if the enumeration form would have caught nothing the landing review missed across the next several implementation records — then the deferral retires.
...
| One deferral with a named observed-failure trigger (MC-5) | done | ledger row; Falsifiability |
