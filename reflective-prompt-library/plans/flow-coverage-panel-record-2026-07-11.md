...
- **P12 Conductor-style DAG executor template** (HCM-024/025, HCM's High priority). Originally deferred on R1 + GOV-004 + the pack's own signal-accounting rule: OpenFugu's `ultra.py --self-test` proves the mechanism is stdlib-scriptable, but no local task had needed dependency-gated fan-out that pipeline/parallel/orchestrator cannot express; HCM's own Socratic question concedes sequential + explicit state-file naming may cover ~everything local. Trigger: first such local task. **Resolved 2026-07-12 (user-directed; recurrence `unknown`):** template added → `flow-control-generator` §Template: DAG Executor; the HCM High-priority dissent is honored, `/batch`-solvable tasks explicitly do not require it.
- **P13 dedicated multi-wave ReMoM template** (HCM-004). Composition note adopted first (Topology Selection: parallel template inside a flow-loop-harness loop with state-file truncation between waves). Trigger: recurrence of real multi-wave runs. **Resolved 2026-07-12 (user-directed; recurrence `unknown`):** dedicated template added → `flow-loop-harness` §Template: Multi-Wave Fan-out; the compose-first note is retained as the default for single-wave work.
- **P15 frozen-core parity items** (RFM-05: reflective-research Blind Spots parity with 05-domain/research.md; historical-banner forward pointers in June plans). Outside pack scope — core skills are frozen and plans-file hygiene is its own deferred item (governance rethink E2). Escalated for a separate review; the panel record is the paper trail. **Resolved 2026-07-11:** review executed → [parity review record](frozen-core-parity-review-2026-07-11.md) — `research.md` acceptance aligned to the STORM none-found contract, four retired-plan banners gained successor pointers; skill and reflection banners no-change.
...
- **Resolved trigger-phrase collision debt:** this panel recorded `pipeline` / `orchestrate` / `loop until` as unmeasured under P7. The successor [P7 decision](p7-pack-routing-decision-2026-07-11.md) added three ROUTE-002/003 groups / 9 phrases, observed 100% pre-tune, and decided no core-router integration.
...
- Writer-critic remains the pack's only advisory-tier gate; its deviation label is honesty, not mitigation. A deterministic companion check (GOV Socratic Q2) was an open design question; **resolved 2026-07-12 (user-directed; recurrence `unknown`)** as guidance — `flow-loop-harness` §Deterministic companion check requires ACCEPT AND a deterministic floor (fail-closed), explicitly staying a partial supplement, not a truth-layer upgrade.
