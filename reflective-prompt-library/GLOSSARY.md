...
32. When editing cross-category library registries, use `assert_library_wide_unique_basenames`, `assert_registry_matches_library_glob`, and `sorted_all_library_prompts` in `prompt_eval_helpers.py`; run the per-registry glob/unique guards (`test_*_library_registry.py`).
