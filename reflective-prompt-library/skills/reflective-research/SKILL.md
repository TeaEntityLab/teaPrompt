...
- A summary's citation is the summary's claim about a source, not the source's evidence: a claim that arrived through a summary keeps the summary in its Source column until the cited page is read, however real the link; then record what the page says beside what the summary said it says.
...
- Each evidence entry names the claim, the source, the attester, the date checked, and which kind of freshness applies: a date to recheck, a tracking event that invalidates it, or an immutable pin (digest, commit, or published text).
